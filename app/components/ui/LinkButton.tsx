...
export function LinkButton({
  ...
  const composedClassName = button({ variant, isDisabled, className });

  if (isDisabled) {
    return (
      <span ref={ref} className={composedClassName} {...buttonProps}>
        {children}
      </span>
    );
  }
  ...
  return (
    <Link ref={ref} to={to} className={composedClassName} {...buttonProps}>
      {children}
    </Link>
  );
}
